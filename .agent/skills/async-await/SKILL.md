...
[public|private|protected] async Task[T]? MethodNameAsync(params...) [Generic type constraints]
{
    [method body with await keyword(s)]
}

```csharp
// Example 1: Async method with return value
...
public AppointmentDto CreateAppointment(CreateAppointmentDto input)
{
    ...
    var doctor2 = _doctorRepository.GetAsync(input.DoctorId2).GetAwaiter().GetResult(); // DON'T DO THIS
    // ...
}
```
...
**ASP.NET Core OR UI Applications calls in Main thread (ConfigureAwait Not Needed):**
```csharp
...
- UI applications (WPF/WinForms/MAUI) calls in Main thread: Don't use ConfigureAwait(false)
...
- [ ] No async void (except event handlers, but then need to study reentrancy)
